...
struct WatchGrammarWritingView: View {
    ...
    @State private var showsSilentKeyboard = false
    ...
    var body: some View {
        List {
            if let referenceDocument {
                Section("考试速查文档") {
                    NavigationLink {
                        WatchMarkdownView(document: referenceDocument)
                    } label: {
                        Label("终极时态与语法速查表", systemImage: "doc.text.fill")
                    }
                }
            }
            ...
            Section("老师题目") {
                ...
                TextField("例如：shopping garage sale", text: $topic, axis: .vertical)
                    .lineLimit(2...4)
                Button {
                    showsSilentKeyboard = true
                } label: {
                    Label("静音英文键盘", systemImage: "keyboard")
                }
            }
        ...
        .sheet(isPresented: $showsSilentKeyboard) {
            WatchSilentEnglishKeyboard(
                text: $topic,
                isGenerating: isSending || requestPending
            ) {
                showsSilentKeyboard = false
                submit()
            }
        }
        .onAppear {
    ...
    private var referenceDocument: WatchDocument? {
        documents.first {
            $0.manifest.title.contains("终极时态与语法速查表")
        }
    }
...
/// A fully local, silent alternative to watchOS Scribble and dictation.
/// It intentionally appends at the end: exam topics are short, and large
/// letter targets are much more reliable than a tiny full QWERTY layout.
struct WatchSilentEnglishKeyboard: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 5) {
                Text(text.isEmpty ? "点击字母输入题目" : text)
                    .font(.caption)
                    .foregroundStyle(text.isEmpty ? .secondary : .primary)
                    .frame(maxWidth: .infinity, minHeight: 38, alignment: .leading)
                    .padding(.horizontal, 6)
                    .background(.gray.opacity(0.16), in: RoundedRectangle(cornerRadius: 7))
                ...
                HStack(spacing: 4) {
                    Button {
                        ...
                        Image(systemName: uppercase ? "shift.fill" : "shift")
        ...
        .navigationTitle("静音键盘")
    }

    private func letterButton(_ letter: Character) -> some View {
        Button {
            let value = String(letter)
            text.append(uppercase ? value.uppercased() : value)
            if uppercase { uppercase = false }
        } label: {
            Text(uppercase ? String(letter).uppercased() : String(letter))
...
struct WatchMarkdownView: View {
    ...
    private var markdownText: AttributedString {
        let source = document.manifest.title.contains("终极时态与语法速查表")
            ? verticalizedTables(in: content)
            : content
        return (try? AttributedString(markdown: source)) ?? AttributedString(source)
    }

    /// GFM tables are useful on iPhone but too wide for a small Watch.
    /// Convert each row to a heading plus labeled fields before rendering.
    private func verticalizedTables(in markdown: String) -> String {
        ...
        while index < lines.count {
            ...
            guard index + 1 < lines.count,
                ...
                output.append(compactReferenceLine(line))
                index += 1
                continue
            }
            ...
            while index < lines.count,
                ...
                for column in 1..<min(headers.count, cells.count) {
                    let value = cells[column]
                        .replacingOccurrences(of: "<br>", with: "\n")
                    guard !value.isEmpty else { continue }
                    output.append("**\(headers[column])：** \(value)")
                }
                output.append("")
                index += 1
            }
    ...
    private func compactReferenceLine(_ line: String) -> String {
        let normalized = line.replacingOccurrences(of: "<br>", with: "\n")
        let trimmed = normalized.trimmingCharacters(in: .whitespaces)
        guard trimmed.count > 18,
              let marker = trimmed.first,
              marker == "═" || marker == "─",
              trimmed.allSatisfy({ $0 == marker }) else {
            return normalized
        }
        return String(repeating: marker, count: 18)
    }
    ...
    private func isTableSeparator(_ line: String) -> Bool {
        let remaining = line.filter {
            !$0.isWhitespace && $0 != "|" && $0 != ":" && $0 != "-"
        }
        return remaining.isEmpty && line.contains("-")
    }
}
